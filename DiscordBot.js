...
class DiscordBot {
    ...
    async postKillWithTimeout(kill, timeout = 30000) {
        ...
        return new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                reject(
                    new Error(
                        `Timeout exceeded for posting kill ${kill.EventId}`
                    )
                );
            }, timeout);
    ...
    async postKill(kill) {
        ...
        if (kill.Victim.Inventory.some((item) => item !== null)) {
            inventoryPath = await imageGenerator.generateInventoryImage(
                kill.Victim
            );
        }
        ...
        if (!this.channel) {
            console.error(
                `Channel ID ${this.config.botChannel} not found in cache.`
            );
            return;
        }
        ...
        try {
            ...
            if (inventoryPath !== null) {
                ...
                await this.channel.send({
                    ...
                    files: [
                        { attachment: inventoryPath, name: "inventory.png" },
                    ],
                });
